...
CuriousReader.prototype.createSlides = function () {
  ...
  for (let i = 0; i < self.children.length; i++) {
    ...
    if (self != undefined) {
      //console.log(this.elementInstances[i][2])
      //this.elementInstances[i][2].listener()
      self.children[i].getElement()[0].children[1].children[0].children[0].innerHTML='<h2>This is the heading</h2>'
      this.elementInstances[i][2].on('timeupdate',function(){
        console.log("Triggered")
      })
      console.log(self.children[i].getElement()[0].children[1].children[0].children[0].innerHTML)
      console.log(self.children)
      for (let j = 0; j < self.slides[i].elements.length; j++) {
        //console.log(self.slides[i].elements[j].action)
        if (self.slides[i].elements[j].action.library == 'H5P.AdvancedText 1.1') {
          //console.log(self.slides[i].elements[j].action.params.text)
          //self.slides[i].elements[j].action.params.text.innerhtml='<h2>This is the header</h2>'
          console.log(self)
          self.slides[i].elements[j].action.params.text='<h2>This is sample</h2>'
          // this.splittedTime = self.slides[i].elements[j].action.params['Each duration and text']
        }

        if (self.slides[i].elements[j].action.library == 'H5P.CRAudio 1.4') {
          //console.log(self.slides[i].elements[j].action.params['Each duration and text'])
          this.splittedTime = self.slides[i].elements[j].action.params['Each duration and text']
        }
        // for(let k=0;k<self.slides[i].elements.length;k++)
        // {
